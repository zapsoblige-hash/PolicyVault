...
let seedCounter = 0;
async function seed(over = {}, registry = REGISTRY) {
  seedCounter += 1;
...
async function buildSpend(signerKp = agentA, params = {}) {
...
test("G10 two-tab: two requests on one predecessor — only ONE finalizes; the second is CLAIM_CONFLICT", async () => {
  await seed();
  const a = await buildSpend(agentA);
  const b = await buildSpend(agentA, { recipient: XO(other) });
  assert.notEqual(a.txId, b.txId);
  const doneA = await wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: signAll(a, agentA) });
  assert.equal(doneA.state, "PREFLIGHT_VERIFIED");
  // second transition on the SAME predecessor outpoint is refused
  await assert.rejects(async () => wr4.finalizeWalletRequestV4({ config, requestId: b.requestId, signedSafeJson: signAll(b, agentA) }), (e) => e.code === "CLAIM_CONFLICT");
});

test("G10 crash/duplicate: finalizing the SAME request twice is refused by the state machine", async () => {
  await seed();
  const a = await buildSpend(agentA);
  await wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: signAll(a, agentA) });
  // duplicate FINALIZE call: request is now PREFLIGHT_VERIFIED, not BUILT
  await assert.rejects(async () => wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: signAll(a, agentA) }), /not BUILT/);
});

test("G10 stale: a request built against a superseded predecessor is STALE at finalize", async () => {
  await seed();
  const a = await buildSpend(agentA);
  ...
  // superseded).
  await seed({ protectedKas: 900n });
  await assert.rejects(async () => wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: signAll(a, agentA) }), (e) => e.code === "STALE");
});

/* ------------------------------------------------------- freeze boundary */

test("G13 freeze: a wallet that mutates an OUTPUT (recipient/amount/change) is rejected (immutability)", async () => {
  await seed();
  const a = await buildSpend(agentA);
  ...
  await assert.rejects(async () => wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: JSON.stringify(signed) }), (e) => e.code === "SIGNATURE_INVALID");
});

test("G13 freeze: a wallet that mutates an INPUT outpoint/sequence is rejected (immutability)", async () => {
  await seed();
  const a = await buildSpend(agentA);
  const signed = JSON.parse(signAll(a, agentA));
...
test("G13 identity: signing with the WRONG key passes immutability but FAILS the production covenant preflight", async () => {
  await seed();
  const a = await buildSpend(agentA);
  ...
  await assert.rejects(async () => wr4.finalizeWalletRequestV4({ config, requestId: a.requestId, signedSafeJson: wrong }), (e) => e.code === "PREFLIGHT_FAILED");
});

/* ----------------------------------------------------------- approvals */

test("G13 approvals: an approval collected for request A does not verify on request B (cross-tx replay)", async () => {
  await seed({ approvers: approvers.map(XO), approvalM: "2" });
  const a = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (6n * KAS).toString(), agentPk: XO(agentA), recipient: XO(recipient), fuel: fuelFor(agentA) }, signerAddress: ADDR(agentA) });
  const b = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (6n * KAS).toString(), agentPk: XO(agentA), recipient: XO(other), fuel: fuelFor(agentA) }, signerAddress: ADDR(agentA) });
  ...
  await assert.rejects(async () => wr4.collectApprovalV4({ config, requestId: b.requestId, approverAddress: ADDR(approvers[0]), signedSafeJson: sigForA }), (e) => e.code === "SIGNATURE_INVALID");
});

test("G13 approvals: unknown approver and duplicate approval are refused", async () => {
  await seed({ approvers: approvers.map(XO), approvalM: "2" });
  const a = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (6n * KAS).toString(), agentPk: XO(agentA), recipient: XO(recipient), fuel: fuelFor(agentA) }, signerAddress: ADDR(agentA) });
  ...
  await assert.rejects(async () => wr4.collectApprovalV4({ config, requestId: a.requestId, approverAddress: ADDR(attacker), signedSafeJson: approverSign(attacker) }), (e) => e.code === "UNKNOWN_APPROVER");
  // valid approval, then duplicate from the same approver
  await wr4.collectApprovalV4({ config, requestId: a.requestId, approverAddress: ADDR(approvers[0]), signedSafeJson: approverSign(approvers[0]) });
  await assert.rejects(async () => wr4.collectApprovalV4({ config, requestId: a.requestId, approverAddress: ADDR(approvers[0]), signedSafeJson: approverSign(approvers[0]) }), (e) => e.code === "DUPLICATE_APPROVAL");
});

/* --------------------------------------------------------- fee/reserve */

test("G13 fee/reserve: browser-supplied reserveConsumed > fee (but below the agent cap) is rejected, not trusted", async () => {
  await seed();
  ...
  await assert.rejects(async () => buildSpend(agentA, { fuel: fuelFor(agentA), reserveConsumedSompi: "50000000" }), (e) => e.code === "RESERVE_OVER_FEE");
  // and a value above the cap is rejected by the per-agent fee cap
  await assert.rejects(async () => buildSpend(agentA, { fuel: fuelFor(agentA), reserveConsumedSompi: (90n * KAS).toString() }), (e) => e.code === "OVER_AGENT_FEE_CAP");
});

/* ---------------------------------------------------------- network gate */

test("G11 network gate: v0.4 requests refuse to build on any non-operational network (Gate R posture)", async () => {
  await seed();
  ...
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config: mainnetish, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (4n * KAS).toString(), agentPk: XO(agentA), recipient: XO(recipient) }, signerAddress: ADDR(agentA) }), /dual-flag unlock/);
  // Unknown networks stay fail-closed.
  const devnetish = { ...config, networkId: "devnet" };
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config: devnetish, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (4n * KAS).toString(), agentPk: XO(agentA), recipient: XO(recipient) }, signerAddress: ADDR(agentA) }), /not an operational/);
});

/* ------------------------------------------------- agent-tree integrity */

test("G2/G13: a manifest whose registry cannot reproduce the covenant agentRoot fails closed on load", async () => {
  const m = await seed();
  ...
  await assert.rejects(async () => loadManifestV4(config, VAULT_ID), (e) => e.code === "REGISTRY_ROOT_MISMATCH");
  // and therefore no operation can be built against it
  await assert.rejects(async () => buildSpend(agentA), (e) => e.code === "REGISTRY_ROOT_MISMATCH" || /registry/.test(e.message));
  void m;
});

/* ------------------------------------------------------- browser trust */

test("G13 browser trust: caller-injected successor/agentRoot/policyNonce fields are ignored (server derives)", async () => {
  await seed();
  const clean = await buildSpend(agentA);
  const injected = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { payAmountSompi: (4n * KAS).toString(), agentPk: XO(agentA), recipient: XO(recipient), successor: { protectedValue: "1" }, agentRoot: "cd".repeat(32), policyNonce: "99" }, signerAddress: ADDR(agentA) });
  assert.equal(injected.successorStateId, clean.successorStateId, "successor is server-derived, not caller-supplied");
});

test("G13 pause: a paused vault refuses agent spends but allows owner unpause", async () => {
  const m = await seed();
  ...
  await assert.rejects(async () => buildSpend(agentA), /ACTIVE|paused/);
  // owner unpause is allowed while paused
  const un = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "ownerUnpause", params: { fuel: fuelFor(owner) }, signerAddress: ADDR(owner) });
  const done = await wr4.finalizeWalletRequestV4({ config, requestId: un.requestId, signedSafeJson: signAll(un, owner) });
