...
test("§21-A/H app-v4.js consumes the canonical session and opens NO second provider", async () => {
...
test("§21 app.js owns the single canonical session API", async () => {
...
test("§3 exactly one Connect KasWare button in the served UI", async () => {
...
before(async () => { await seedV41(); });
const fuel = { outpoint: { transactionId: "43".repeat(32), index: 1 }, amount: (100n * KAS).toString(), scriptPublicKeyHex: `20${XO(OWNER)}ac` };

test("§10/§21-B agent action binds the connected wallet to the acting agentPk", async () => {
  // The connected wallet IS agent A -> authorized.
  const ok = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { agentPk: XO(AGENT_A), recipient: XO(RECIP), payAmountSompi: (4n * KAS).toString() }, signerAddress: ADDR(AGENT_A) });
  ...
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { agentPk: XO(AGENT_A), recipient: XO(RECIP), payAmountSompi: (4n * KAS).toString() }, signerAddress: ADDR(AGENT_B) }),
    /NOT_AGENT|AUTHORIZATION|agent/i, "agent B cannot spend as agent A"
  );
});

test("§21-D unrelated wallet gets no privileged authorization", async () => {
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "agentSpend", params: { agentPk: XO(AGENT_A), recipient: XO(RECIP), payAmountSompi: (4n * KAS).toString() }, signerAddress: ADDR(UNRELATED) }),
    /NOT_AGENT|AUTHORIZATION|agent/i, "unrelated wallet cannot spend"
  );
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "ownerPause", params: { fuel }, signerAddress: ADDR(UNRELATED) }),
    /NOT_OWNER|AUTHORIZATION|owner/i, "unrelated wallet cannot perform owner ops"
  );
});

test("§10 owner action binds the connected wallet to the vault owner", async () => {
  const ok = await wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "ownerPause", params: { fuel }, signerAddress: ADDR(OWNER) });
  assert.ok(ok.requestId, "owner may pause");
  await assert.rejects(async () => wr4.buildWalletRequestV4({ config, vaultId: VAULT_ID, action: "ownerPause", params: { fuel }, signerAddress: ADDR(AGENT_A) }),
