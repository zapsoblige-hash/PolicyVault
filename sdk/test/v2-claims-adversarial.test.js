...
test("second claim on the same predecessor fails CLAIM_CONFLICT and preserves the first", () => {
  const config = tempConfig();
  const o = outpoint();
  const tx1 = "aa".repeat(32);
  const tx2 = "bb".repeat(32);
  claimTransition(config, base(o, tx1));
  assert.throws(() => claimTransition(config, base(o, tx2)), (e) => e.code === "CLAIM_CONFLICT" && /already claimed/.test(e.message));
  assert.equal(loadTransitionClaim(config, o).txId, tx1, "original claim must be untouched");
});

test("exact same-transaction retry also conflicts (reconcile owns resolution, not retry)", () => {
  const config = tempConfig();
  const o = outpoint();
  const tx1 = "aa".repeat(32);
  claimTransition(config, base(o, tx1));
  assert.throws(() => claimTransition(config, base(o, tx1)), (e) => e.code === "CLAIM_CONFLICT");
});

test("multi-process race: exactly one of 6 concurrent claimants wins", () => {
  const config = tempConfig();
  const o = outpoint();
  const script = `
...
test("corrupted claim file fails closed (loud error, no silent advance)", () => {
  const config = tempConfig();
  const o = outpoint();
  claimTransition(config, base(o, "aa".repeat(32)));
  fs.writeFileSync(transitionClaimPath(config, o), "{ not json !!");
  assert.throws(() => loadTransitionClaim(config, o), /corrupt/);
  // And a fresh claim on the same outpoint still conflicts (file exists).
  assert.throws(() => claimTransition(config, base(o, "bb".repeat(32))));
});

test("truncated claim file fails closed", () => {
  const config = tempConfig();
  const o = outpoint();
  claimTransition(config, base(o, "aa".repeat(32)));
...
test("submission claim is idempotent for the same txid, distinct txids coexist", () => {
  const config = tempConfig();
  const tx1 = "aa".repeat(32);
  const tx2 = "bb".repeat(32);
  claimSubmission(config, { txId: tx1, vaultId: "11".repeat(32), action: "delegateSpend" });
  claimSubmission(config, { txId: tx1, vaultId: "11".repeat(32), action: "delegateSpend" }); // no throw
  claimSubmission(config, { txId: tx2, vaultId: "11".repeat(32), action: "delegateSpend" });
  assert.ok(fs.existsSync(submissionClaimPath(config, tx1)));
  assert.ok(fs.existsSync(submissionClaimPath(config, tx2)));
});

test("post-confirmation staleness: consumed predecessor stays claimed; the successor outpoint is freshly claimable", () => {
  const config = tempConfig();
  const oldOutpoint = outpoint();
  const confirmedTx = "aa".repeat(32);
  // First transition claimed and (conceptually) confirmed.
  claimTransition(config, base(oldOutpoint, confirmedTx));
  ...
  assert.throws(() => claimTransition(config, base(oldOutpoint, "cc".repeat(32))), (e) => e.code === "CLAIM_CONFLICT");
  ...
  claimTransition(config, base(successorOutpoint, "dd".repeat(32)));
  assert.equal(loadTransitionClaim(config, successorOutpoint).txId, "dd".repeat(32));
});

test("legacy claim without an expected record cannot advance state (shape check)", () => {
  const config = tempConfig();
  const o = outpoint();
  claimTransition(config, { ...base(o, "aa".repeat(32)), expected: undefined });
  const claim = loadTransitionClaim(config, o);
