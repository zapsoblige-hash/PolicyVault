...
/*
 * Exact live-state compiler for the PolicyVault v0.6 ATOMIC-COMPOSABILITY
 * TOKEN CONTROLLER covenant (contracts/PolicyVault.v0.6.sil — CANDIDATE, not
 * byte-frozen). Same write-or-assert + silverc pipeline as v0.5: the 6
 * mutable initializers are templated with exact live values; the 12
 * constructor args carry the immutable pins (owner, vaultId,
 ...
 *
 * Status: IMPLEMENTED (SDK). Exercised by sdk/test/contract-compiler-v6.test.js
...
const { computeStateIdV6, CONTRACT_VERSION_V6, resolveV6Abi, normalizeTemplateV6, normalizeStateV6 } = require("./vault-state-v6");
const { blake2bHex } = require("../../core/assets/blake2b");

function fail(message) {
  throw new Error(`contract-compiler-v6: ${message}`);
}
...
function buildLiveStateSourceV6(originalSource, state) {
  let source = originalSource;
  const swaps = [
    ["int feeReserve = initFeeReserve;", `int feeReserve = ${state.feeReserve};`, "feeReserve"],
    ["int swapPrincipal = initSwapPrincipal;", `int swapPrincipal = ${state.swapPrincipal};`, "swapPrincipal"],
    ...
    ["byte[32] swapRoot = initSwapRoot;", `byte[32] swapRoot = 0x${state.swapRoot};`, "swapRoot"],
...
/*
 * v0.6 constructor (12 args, declaration order — vaultId at index 1, where
 ...
 * templateSuffixLen, initAgentRoot, initFeeReserve, initSwapRoot,
 * initSwapPrincipal.
 */
function constructorArgsV6(template, state) {
  return [
    ...
    bytesArg(state.swapRoot, "initSwapRoot"),
    intArg(state.swapPrincipal, "initSwapPrincipal")
  ];
}
...
function runSilverc({ silvercPath, sourcePath, constructorArgsPath, outputPath }) {
  ...
  if (result.status !== 0 || !fs.existsSync(outputPath)) {
    fail(
      ["silverc v0.6 state compilation failed", `source: ${sourcePath}`, `exit: ${result.status}`, result.stdout?.trim() ?? "", result.stderr?.trim() ?? ""]
        .filter(Boolean)
        .join("\n")
    );
  }
...
function compileExactStateV6({ config, template: templateInput, state: stateInput, contractVersion }) {
  const abi = resolveV6Abi(contractVersion ?? CONTRACT_VERSION_V6);
  const template = normalizeTemplateV6(templateInput);
  const state = stateInput.recoveryParse === true ? stateInput : normalizeStateV6(stateInput);
  ...
  const liveSource = buildLiveStateSourceV6(originalSource, state);
  const argsJson = JSON.stringify(constructorArgsV6(template, state), null, 2) + "\n";
