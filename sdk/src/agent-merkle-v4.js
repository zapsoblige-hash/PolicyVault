...
const AGENT_LEAF_DOMAIN = Buffer.from([0x50, 0x56, 0x34, 0x01]);
const AGENT_PADDING_DOMAIN = Buffer.from([0x50, 0x56, 0x34, 0x00]);
...
const PADDING_LEAF_HEX = PADDING_LEAF.toString("hex");
...
function agentLeafPreimage(policyInput) {
  ...
  const preimage = Buffer.concat([
    ...
    Buffer.from(p.agentRecipientRoot, "hex")
  ]);
...
/* Canonical agent-leaf hash (Buffer). */
 ...
 * Buffer levels for proof generation.
 */
...
function foldLeafV4(leafBuffer, siblingsHex, pathBits) {
  if (!Buffer.isBuffer(leafBuffer) || leafBuffer.length !== 32) {
