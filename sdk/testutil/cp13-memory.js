...
const shared={"path":path,"fs":{},"os":{},"crypto":require("node:crypto"),"./store":{getStore:()=>store,Categories},"./org-root-lock":require(WT+"/sdk/src/org-root-lock"),"./config":{assertOperationalNetwork:()=>{},assertGenerationMainnetCreatable:()=>{},assertGenerationMainnetOperable:()=>{}},"./amounts":pure("amounts"),"./vault-state":pure("vault-state"),"./agent-merkle-v5":pure("agent-merkle-v5"),"./recipient-merkle-v3":pure("recipient-merkle-v3"),"./chain":chain,"./manifest":{VaultStatus:stat,TERMINAL_STATUSES:new Set(["RECOVERED"])},"./address-identity":{addressForXOnlyPubkey:(c,x)=>x},"./submission-claim":claimModule,"./audit":{},"./contract-compiler-v7":{compileExactStateV7:({state})=>({scriptSha256:state.agentRoot===op.stateAfter.state.agentRoot?H("bb"):H("aa"),scriptHex:"11"})},"./build-cache":{},"./vm-preflight":{},"./frozen-tx-v3":{},"./approval-package-v4":{},"./vault-builders-v7":{},"./wallet-submit-v4":{finalTxToWasm:()=>({finalize:()=>txid}),isDefinitiveSubmitRejection:()=>false},
/* v0.7 mainnet enablement (2026-09-10): the shared root pipeline dispatches rooted vaults by PROFILE through
 * ./rooted-kas-profile-v7 (and reconcile-v7 looks a vault up in ./manifest-v7-kas first). These memory fixtures are the
 * FROZEN payment profile only: the KAS predicates answer "not KAS" and every KAS-only entry is a closed stub that throws
 * if ever reached — a future change routing a payment-profile record through the KAS branch fails here loudly. */
"./rooted-kas-profile-v7":{CONTRACT_VERSION_V7_KAS:"policyvault-0.7-kas",OWNER_OP_ACTIONS_KAS:new Set(),isKasBuild:()=>false,isKasVaultRecord:()=>false,loadManifestV7Kas:async()=>null,loadKasWalletRequest:async()=>null,unfinishedDelegateRequests:async()=>[],...Object.fromEntries(["ensureBuildDirKas","classifyVaultRecord","expectedVaultSuccessorDoc","persistManifestV7Kas","buildOrgManifest","verifyOrgManifest","buildOwnerOp","prepareOwnerOpParams","resolveOwnerOpAuthority","finalizeOwnerOp","vaultSuccessorIndexOf"].map((k)=>[k,()=>{throw Error("STUB_KAS_UNUSED:"+k)}]))},
"./manifest-v7-kas":{loadManifestV7Kas:async()=>null},
"./wallet-requests-v7-kas":{reconcileKasVault:()=>{throw Error("STUB_KAS_UNUSED:reconcileKasVault")}}};
/* RC33-ID-01 (2026-09-11): the genesis writers arbitrate the vault identity through ./vault-identity, which needs only the
 * store facade above (read / createExclusive / listValues) and the pure core canonicalizer — loaded through the same
 * closed loader so its ./store import is THIS memory store (a real module, no stub; unknown imports still fail loudly). */
/* RC35-REC-01 (2026-09-11): the shared submit-outcome classifier (pure) is imported by ./vault-identity and every submitter; loaded through the closed loader (unknown imports still fail loudly) */
shared["./submission-classification"]=load("submission-classification.js");
shared["./vault-identity"]=load("vault-identity.js");
