...
/* RC33-ID-01 (2026-09-11): the create-only sibling the genesis completion now uses — same memory map, create-or-match semantics */
createManifestV4:async(c,m)=>{const k=rootKey(c);if(manifests.has(k))return {manifest:structuredClone(manifests.get(k)),outcome:"ALREADY_PRESENT"};await m4.persistManifestV4(c,m);return {manifest:structuredClone(manifest),outcome:"CREATED"};}};
...
/* RC33-ID-01 (2026-09-11): the genesis build / submit paths arbitrate the vault identity through the REAL ./vault-identity module, loaded against the same actual-store facade (no stub) */
/* RC35-REC-01 (2026-09-11): the shared classifier (pure) and the settlement / recovery primitives are REAL modules loaded against the same facades (no stub) */
const submissionClassification=loadAux("submission-classification.js",{});deps["./submission-classification"]=submissionClassification;wrImports["./submission-classification"]=submissionClassification;
const genesisRecovery=loadAux("genesis-recovery.js",{"./store":actualStoreFacade,"./chain":deps["./chain"],"./submission-claim":deps["./submission-claim"],"./submission-classification":submissionClassification});deps["./genesis-recovery"]=genesisRecovery;wrImports["./genesis-recovery"]=genesisRecovery;
const vaultIdentity=loadAux("vault-identity.js",{"./store":actualStoreFacade,"../../core/intent/canonical":require(WT+"/core/intent/canonical"),"./submission-classification":submissionClassification});wrImports["./vault-identity"]=vaultIdentity;deps["./vault-identity"]=vaultIdentity;
