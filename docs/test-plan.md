...
| **GLOBAL VAULT-RECORD UNIQUENESS (RC33-ID-01, 2026-09-11)** — SDK / CRASH-RECOVERY / CONCURRENCY | `sdk/test/vault-identity-uniqueness.test.js` — the independent reviewer's six regression cases (RED 0/6 on the original rc33 build source `efe9372`, retained under `docs/postlaunch/v07-enablement-evidence/codex-rc33-review/checkpoint-02/`) + controls: fresh identities, same-request idempotence (record byte-identical, nothing rebroadcast), "same identity alone" refused, EVERY caller-selectable generation (v0.4.1 / v0.5 / v0.6 / v0.7-payment / v0.7-payment-hd / v0.7-kas) refuses an occupied identity before build, v0.4.1 sign+submit / atomic completion / reconcile replay, every other generation's completion atomic (competing writer between check and create never replaced), restart recovery, unreadable record occupies, and the internal review's F1 cases — a PRE-CORRECTION pair of drafts naming one identity signed / submitted CONCURRENTLY: exactly one proceeds (one broadcast), the other refused `VAULT_ID_IN_USE` and left unsigned (v0.4.1, v0.5, v0.6, v0.7-payment, v0.7-payment-hd, v0.7-kas) | 18 |
| **Store arbiter (real backends)** — SDK / CONCURRENCY | `sdk/test/vault-identity-store-arbiter.test.js` — 12 genuinely concurrent create-only writers on one key against the REAL JSON `link()`/`EEXIST` arbiter and the REAL PostgreSQL `INSERT … ON CONFLICT DO NOTHING` arbiter (exactly one wins), create-or-match semantics, reservations of every kind and phase, per-identity lock, unreadable / mis-bound records occupy | 2 (PG skipped cleanly without `POLICYVAULT_TEST_PG_*`) |
| **Served boundary** — API / HOSTED (real server + real PostgreSQL) | `sdk/test/hosted-vault-identity-uniqueness.test.js` — `POST /org-roots/:id/vaults` (kas / payment / hd), `POST /wallet/v4/create`, `POST /wallet/v5/create`: closed `409 VAULT_ID_IN_USE`, nothing persisted, nothing disclosed; the KAS signature route refuses an occupied identity and leaves the request BUILT; fresh identities create; tenancy first (a foreign principal never reaches the check) | 4 |
| **SUBMISSION-OUTCOME CLASSIFICATION (RC35-REC-01, 2026-09-11)** — UNIT | `sdk/test/submission-classification-rc35-rec01.test.js` — the shared three-way classifier: REJECTED (bound envelope, this attempt's txid, every transport form), ALREADY_KNOWN (the exact RejectAlreadyAccepted / RejectDuplicate / RejectDuplicateOrphan answers of the retained node source, never definitive), AMBIGUOUS (transport, another txid, unbound envelope); persisted-negative helpers (ESTABLISHED / FALSE_NEGATIVE / UNKNOWN) | 4 |
| **KAS GENESIS OUTCOME + RECOVERY (RC35-REC-01)** — SDK / CRASH-RECOVERY | `sdk/test/kas-genesis-outcome-recovery-rc35-rec01.test.js` — the reviewer's already-accepted case (observed, one broadcast, same request settles), already-in-mempool then observation, genuine rejection with output-absent proof (claim released, build reservation kept, commit reservation released), contradiction / other-txid / transport keep the claim, ALREADY-PERSISTED false negative recovered by observation (arbiter blocks it at commit; protected then completed; identical request / txid / signed bytes; nothing rebroadcast), established historical negative unchanged, recovery under the creation kill switch (mainnet-labelled), delegate path (already-in-mempool observed; rejection settled only with chain confirmation; contradiction completed by reconcile) | 8 |
| **ROOT / ROOTED GENESIS RECOVERY (RC35-REC-02 + legacy F4)** — SDK / CRASH-RECOVERY | `sdk/test/org-root-genesis-recovery-rc35-rec02.test.js` — lost response completed by the same submit entry without a root record (one broadcast; root record / receipt / audit; claim released), reconcile of a missing root (GENESIS_PENDING → ADVANCED; ROOT_NOT_FOUND without a request), crash between root record and request write replayed idempotently (root byte-identical, never overwritten), repeated observation idempotent, advanced root preserved by receipt lineage, a different record never replaced, recovery with root creation disabled (kill switch), persisted false negative protected then completed, genuine negative with proof, already-in-mempool observed, rooted-vault (v0.7-payment) genesis lost response + false negative | 8 |
| **LEGACY GENESIS RECOVERY (F4: v0.5 / v0.6 / v0.7-payment-hd)** — SDK / CRASH-RECOVERY | `sdk/test/legacy-genesis-recovery-rc35-f4.test.js` — per family: lost response completed by observation through the same request, persisted false negative protected then completed, established negative unchanged, genuine rejection with output-absent proof, already-in-mempool observed, mainnet-labelled recovery refused (never mainnet-authorized); HD false negative protected then completed | 3 |
| **HEADLESS IDENTITY RESERVATION (RC35-RES-01)** — SDK / CRASH-RECOVERY (real WASM generator, real signatures, mock node) | `sdk/test/headless-identity-reservation-rc35-res01.test.js` — v0.1 and v0.2: durable record before claim and broadcast, lost response keeps the claim and reserves the identity in both phases — also in a FRESH PROCESS — then observation-only recovery (no rebroadcast), fresh identities keep working; genuine rejection settled with proof while the identity stays reserved (never recycled); already-accepted observed; unresolved stays protected; LEGACY retained claim reserves (fresh process too), is listed, refuses a missing / mismatched definition, leaves an unobserved claim untouched, completes from the ORIGINAL definition with the covenant id read from the chain | 5 |
| **HOSTED ROOT-GENESIS RECOVERY ACCESS (RC35-REC-02)** — API / HOSTED (real server + real PostgreSQL + live testnet-10 node subtest) | `sdk/test/hosted-org-root-genesis-recovery-rc35-rec02.test.js` — a pending root genesis (no root record, no root ACL) is reachable only by its creator: foreign principal 404 at GET / submit / root / reconcile before any lock or node call; anonymous 401; creator reads it; reconcile stays root-scoped (404); the creator's submit reaches the observation-only recovery on the real node and preserves request / claim / txid / signed bytes when unobserved; SDK-level completion; root ACL applies once the record exists | 1 (+1 live-node subtest) |
