...
//! production PolicyVault.v0.4.sil. Never the in-process library encoder —
...
fn owner_op(owner_kp: &Keypair, func: &str, prev: &S, succ: Option<&S>, recover: bool) -> Result<(), TxScriptError> {
    let owner = xonly(owner_kp);
    let active = compile_state(&owner, prev);
    ...
    let mut call = serde_json::json!({ "contractVersion": "policyvault-0.4", "function": func, "signature": hexs(&osig) });
    if !recover {
...
#[test]
fn enc4_agent_spend_below_threshold() {
...
fn enc4_owner_ops_and_recover() {
    ...
    let (a1, a2, a3) = (deterministic_keypair(20), deterministic_keypair(21), deterministic_keypair(22));
    // setAgentRoot
    assert!(owner_op(&owner, "ownerSetAgentRoot", &prev, Some(&S { agent_root: [0x77; 32], policy_nonce: 1, ..prev.clone() }), false).is_ok(), "setAgentRoot");
    // setApprovers
    assert!(owner_op(&owner, "ownerSetApprovers", &prev, Some(&S { approvers: approver_set(&[&a1, &a2, &a3]), approval_m: 2, policy_nonce: 1, ..prev.clone() }), false).is_ok(), "setApprovers");
    // topUp / topUpReserve
    assert!(owner_op(&owner, "ownerTopUp", &prev, Some(&S { protected: prev.protected + 100 * KAS, ..prev.clone() }), false).is_ok(), "topUp");
    assert!(owner_op(&owner, "ownerTopUpReserve", &prev, Some(&S { reserve: prev.reserve + 3 * KAS, ..prev.clone() }), false).is_ok(), "topUpReserve");
    // pause / unpause
    assert!(owner_op(&owner, "ownerPause", &prev, Some(&S { paused: 1, ..prev.clone() }), false).is_ok(), "pause");
    let paused = S { paused: 1, ..prev.clone() };
    assert!(owner_op(&owner, "ownerUnpause", &paused, Some(&prev), false).is_ok(), "unpause");
    // recover
    assert!(owner_op(&owner, "ownerRecover", &prev, None, true).is_ok(), "recover");
}

#[test]
fn enc4_rollover_and_reserve() {
...
#[test]
fn enc4_mut_insufficient_approvals() {
    let (a1, a2, a3) = (deterministic_keypair(20), deterministic_keypair(21), deterministic_keypair(22));
    let slots = approver_set(&[&a1, &a2, &a3]);
    let (owner, a, recipient, prev, pa, psibs, pbits, _r) = setup(50 * KAS, slots, 2);
...
#[test]
fn enc4_mut_forged_successor_root() {
    let (owner, a, recipient, prev, pa, psibs, pbits, _r) = setup(100_000 * KAS, [ZERO32; 10], 0);
...
#[test]
fn enc4_mut_terminal_shape_mismatch() {
    ...
    let call = serde_json::json!({ "contractVersion": "policyvault-0.4", "function": "ownerRecover", "signature": hexs(&osig) });
