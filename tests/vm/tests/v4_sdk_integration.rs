//! VM layer — v0.4 SDK PRODUCTION-BYTE integration (Checkpoint E §E10/§E11).
//!
//! Runs the ACTUAL PolicyVault SDK (node: vault-state-v4, the exact-state
//! compiler via silverc, agent-merkle-v4, recipient-merkle-v3,
//! vault-transitions-v4, compute-budget-v4, frozen-tx-v3,
//! approval-package-v4, vault-builders-v4, the real pv_call_encoder AND
//! pv_tx_probe binaries) to construct fully-finalized v0.4 transactions
//! with real Schnorr signatures, then executes every emitted vector's
...
//! sufficient for every supported shape (incl. agent depth 12 + recipient
//! depth 16 + 10-of-10 + reserve). Reject vectors — the §E11 negative
//! mutation matrix — must be rejected by consensus.
...
fn parse_vector_tx(v: &serde_json::Value) -> (Transaction, Vec<UtxoEntry>) {
    assert_eq!(ju64(&v["version"]), 1, "v0.4 transactions are version 1");
...
fn load_vectors() -> Vec<Vector> {
    ...
    assert!(vectors.len() >= 40, "expected a full vector matrix, got {}", vectors.len());
    vectors
}
...
#[test]
fn v4_sdk_vectors_execute_on_production_covenant() {
    ...
    for v in &vectors {
        if v.expect_accept {
            ...
            println!("SDKVEC4 {} accept used_units={units} required_budget={required} committed={}", v.name, v.committed_budget);
            accepted += 1;
        } else {
            ...
            println!("SDKVEC4 {} rejected as required", v.name);
            rejected += 1;
        }
    }
    println!("SDKVEC4 total accepted={accepted} rejected={rejected}");
    assert!(accepted >= 15 && rejected >= 15, "unexpected vector distribution: {accepted} accepts, {rejected} rejects");
}

/// The true worst case (agent depth 12 + recipient depth 16 + 10-of-10 +
/// reserve) must carry committed budget >= 132 and execute under
/// production pricing (Checkpoint-D measurement: 1,318,131 script units).
#[test]
fn v4_sdk_worst_case_budget() {
    let vectors = load_vectors();
    let v = vectors.iter().find(|v| v.name == "worst_agent12_recip16_10of10").expect("worst-case vector present");
    assert!(v.committed_budget >= 132, "worst case must commit >= 132, got {}", v.committed_budget);
    let (res, units) = execute_input_measured_priced(v.tx.clone(), v.entries.clone(), 0, 1000);
    assert!(res.is_ok(), "worst case must execute: {res:?}");
    println!("SDKVEC4 worst used_units={units} committed={}", v.committed_budget);
    assert!(units > 1_000_000, "worst case should consume >1M script units (sanity), got {units}");
}

/// ownerRecover from a malformed predecessor with an EMPTY fee reserve —
/// the SDK's break-glass construction must execute: funds are never
/// operationally trapped where consensus allows recovery.
#[test]
fn v4_sdk_recover_zero_reserve_executes() {
    let vectors = load_vectors();
    let v = vectors.iter().find(|v| v.name == "recover_zero_reserve_malformed").expect("zero-reserve recover vector present");
    let res = execute_input_with_covenants(v.tx.clone(), v.entries.clone(), 0);
    assert!(res.is_ok(), "SDK-built recovery from malformed/zero-reserve state must execute: {res:?}");
}
