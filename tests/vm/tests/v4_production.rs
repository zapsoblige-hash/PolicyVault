//! VM layer — PRODUCTION PolicyVault.v0.4.sil driven through the real
...
//!
//! FROZEN ABI: docs/covenant-spec-v0.4.md. State 441 B (17 mutable
//! fields), agent leaf 124-byte preimage, agent tree depth <= 12,
//! recipient depth <= 16, 10 approver slots, per-agent fee cap.
...
fn dflt<'a>(owner: [u8; 32], prev: S, agents: Vec<Agent>, target: usize, claim: Agent, signer: &'a Keypair, recipient: &'a Keypair) -> Spend<'a> {
    Spend {
        ...
        seq_override: None, lock_override: None,
    }
}
...
fn run(c: Spend) -> (Result<(), TxScriptError>, u64) {
    ...
    let active = compile(&c.owner, &prev);
    let succ_c = compile(&c.owner, &succ);
...
fn v4p_measure_production_budgets() {
    ...
    let measure_owner = |label: &str, func: &str, succ: Option<&S>, recover: bool| {
        let active = compile(&owner, &prev);
        ...
        let args = if recover { vec![Vec::<Expr<'static>>::new().into(), Expr::bytes(osig)] } else { vec![state_arg(succ.unwrap()), Expr::bytes(osig)] };
    ...
    measure_owner("owner_setAgentRoot", "ownerSetAgentRoot", Some(&S { agent_root: [0x77; 32], policy_nonce: 1, ..prev.clone() }), false);
    measure_owner("owner_recover", "ownerRecover", None, true);
}
...
fn v4p_owner_ops() {
    ...
    let run_owner = |func: &str, succ: S| -> Result<(), TxScriptError> {
        let active = compile(&owner, &prev);
        let succ_c = compile(&owner, &succ);
    ...
    // setAgentRoot: nonce +1, value preserved
    assert!(run_owner("ownerSetAgentRoot", S { agent_root: [0x77; 32], policy_nonce: 1, ..prev.clone() }).is_ok(), "setAgentRoot");
    assert_rejected(run_owner("ownerSetAgentRoot", S { agent_root: [0x77; 32], ..prev.clone() }), "setAgentRoot must bump nonce");
    assert_rejected(run_owner("ownerSetAgentRoot", S { agent_root: [0x77; 32], protected: prev.protected + KAS, policy_nonce: 1, ..prev.clone() }), "setAgentRoot cannot change value");
    assert_rejected(run_owner("ownerSetAgentRoot", S { agent_root: [0x77; 32], reserve: prev.reserve + KAS, policy_nonce: 1, ..prev.clone() }), "setAgentRoot cannot change reserve");
    // setApprovers
    assert!(run_owner("ownerSetApprovers", S { approvers: approver_set(&[&a1, &a2, &a3]), approval_m: 2, policy_nonce: 1, ..prev.clone() }).is_ok(), "setApprovers");
    assert_rejected(run_owner("ownerSetApprovers", S { approvers: { let mut x = [ZERO32; 10]; x[0] = xonly(&a1); x[1] = xonly(&a1); x[2] = xonly(&a3); x }, approval_m: 1, policy_nonce: 1, ..prev.clone() }), "duplicate approvers");
    assert_rejected(run_owner("ownerSetApprovers", S { approvers: approver_set(&[&a1, &a2, &a3]), approval_m: 4, policy_nonce: 1, ..prev.clone() }), "M>active");
    // topUp principal only
    assert!(run_owner("ownerTopUp", S { protected: prev.protected + 100 * KAS, ..prev.clone() }).is_ok(), "topUp");
    assert_rejected(run_owner("ownerTopUp", S { protected: prev.protected + 100 * KAS, reserve: prev.reserve + KAS, ..prev.clone() }), "topUp cannot touch reserve");
    // topUpReserve only
    assert!(run_owner("ownerTopUpReserve", S { reserve: prev.reserve + 3 * KAS, ..prev.clone() }).is_ok(), "topUpReserve");
    assert_rejected(run_owner("ownerTopUpReserve", S { reserve: prev.reserve + 3 * KAS, protected: prev.protected + KAS, ..prev.clone() }), "topUpReserve cannot touch principal");
    // pause/unpause
    assert!(run_owner("ownerPause", S { paused: 1, ..prev.clone() }).is_ok(), "pause");
    // pause cannot reset nonce/agentRoot
    assert_rejected(run_owner("ownerPause", S { paused: 1, agent_root: [0x88; 32], ..prev.clone() }), "pause cannot change agentRoot");
}
...
fn d_byte_facts_and_state_layout() {
    ...
    assert_eq!(c.script.len(), 18_839, "redeem script size must be the frozen 18,839 bytes");
    assert_eq!(c.state_layout.len, 441, "state region must be 441 bytes");
