...
## fullscale-rc40 candidate auth and recovery corrections
...
This is a proposed replacement, pending independent acceptance and activation. A historical rejection string and current output absence are insufficient to release a submission claim. Settlement of an actual node rejection binds the retained rejection and original submission anchor to complete acceptance history, repeated exact funding, mempool and output observations, and a stable sink. An eligible stale ordinary request with transport ambiguity uses a separate original-request/transaction/anchor nonacceptance proof for NOT_BROADCAST; no rejection is invented. Unproved historical negatives and known-accepted node answers cannot enter that ordinary fallback. Incomplete original evidence or contradictory observations remain protected. Same-request observation recovery does not rebuild, sign or rebroadcast. Retained v0.4.1 transitions stay protected until exact original effect observation and durable completion bookkeeping; malformed journals, inconsistent script/version observations, and unproved historical lifecycle labels cannot reopen ordinary admission.

Atomic create-only completion preserves existing records; idempotent recovery accepts only the same genesis outcome or its validated lineage. The process-local lock does not provide multi-process serialization: one service writer per data root remains required. Mainnet creation containment does not itself pause other wallet operations.

Source checks recorded for this candidate: Focused RC40 source checks 94/94; new foreign-completion and existing transition recovery coverage, no failures/skips. RC38 full and RC39 affected results remain historical evidence. Independent exact-image review, provider activation, public delivery and human acceptance are separate evidence. A completed automation check does not substitute for any of those steps.
