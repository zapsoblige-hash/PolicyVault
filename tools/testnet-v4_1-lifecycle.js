...
/*
 * PolicyVault v0.4.1 LIVE testnet-10 lifecycle (Checkpoint H2 automated backbone).
 *
 * Identical to the v0.4 lifecycle tool, but the vault is created at
 * contractVersion policyvault-0.4.1 (the standardness-corrected covenant, 13
 * static sig-ops), so it RELAYS on a default node where v0.4 was rejected. All
 * transitions inherit v0.4.1 from the manifest. This is the AUTOMATED backbone
 * (dev signer, allowed by the H2 directive §5 for automated tests); the REAL
 * KasWare owner/agent/approver acceptance is a separate human step (§5/§6).
 *
...
const OUT = process.argv[2] || "/tmp/pv41-live-evidence.json";
const DATA_ROOT = process.env.PV_LIVE_DATA_ROOT || "/tmp/pv41-live-data";
...
async function main() {
  ...
  async function doAction(label, { vaultId, action, params, signerKey, approverKeys = [] }) {
    ...
    if (built.contractVersion !== V4_1) throw new Error(`${label}: request version ${built.contractVersion} != ${V4_1}`);
  ...
  async function auditRoots(label, vaultId) {
    ...
    if (!m.live) {
      // terminal (RECOVERED/closed): no live successor to audit.
      const audit = { label, terminal: true, status: m.status, live: null };
      evidence.rootAudits.push(audit);
      log(`  root audit OK (terminal: status ${m.status}, no live successor)`);
      return;
    }
  ...
  try {
    ...
    const mGen0 = await loadManifestV4(config, vaultId);
    if (mGen0.contractVersion !== V4_1) throw new Error(`genesis manifest version ${mGen0.contractVersion} != ${V4_1}`);
    log(`GENESIS CHAIN_VERIFIED vault=${vaultId.slice(0,12)} txid=${genResult.txId} version=${mGen0.contractVersion}`);
    evidence.vaultId = vaultId;
    evidence.contractVersion = V4_1;
    evidence.transactions.push({ label: "genesis", action: "createVault", txId: genResult.txId, contractVersion: mGen0.contractVersion, protected: (100n * KAS).toString(), reserve: (5n * KAS).toString() });
